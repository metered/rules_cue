...
def _cue_import_action(ctx, package, inputs, output):
...
def _cue_import_impl(ctx):
    ...
    _cue_import_action(ctx, package, srcs, cue_out)
...
def cue_import(*, name, values=None, **kwargs):
    _json_values = None
    if values != None:
        _json_values = _json_encode(values)
        kwargs["json"] = _json_values

    _cue_import(name = name, **kwargs)
